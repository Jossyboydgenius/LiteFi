...
// POST - Create salary car loan application
export async function POST(request: NextRequest) {
  try {
    ...
    // Validate input
    const validatedData = salaryCarLoanSchema.parse(body);
    ...
    await prisma.loanApplicationLog.create({
      data: {
        ...
        notes: 'Salary car loan application created',
        metadata: {
          loanType: 'SALARY_CAR',
    ...
    return NextResponse.json({
      success: true,
      loanApplication,
      message: 'Salary car loan application submitted successfully',
    }, { status: 201 });
  } catch (error) {
    ...
    console.error('Create salary car loan application error:', error);
    return NextResponse.json(
...
// GET - Fetch user's salary car loan applications
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('Fetch salary car loan applications error:', error);
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    );
